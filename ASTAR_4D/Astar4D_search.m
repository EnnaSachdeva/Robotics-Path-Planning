function [is_solved, timeSpent, closedList, expandedNodes, numExpandedNodes] = Astar4D_search(map, startNode, goalNode, epsilon, currentTime, runTime)
...
tic;
timeSpent = 0;
[start_state_x, start_state_y, start_state_vx, start_state_vy] = dynamic_state_from_index(map, startNode);
[goal_state_x, goal_state_y, goal_state_vx, goal_state_vy] = dynamic_state_from_index(map, goalNode);

start_state = [start_state_x, start_state_y, start_state_vx, start_state_vy]; % coordinates
goal_state = [goal_state_x, goal_state_y, goal_state_vx, goal_state_vy]; % coordinates
...
closedList = [];
openList = [start_state_x, start_state_y, start_state_vx, start_state_vy, G_cost(start_state_x, start_state_y), F_cost(start_state_x, start_state_y), 0];
...
while (~isempty(openList))  % parent pos ~= Target
    ...
    currentNode = dynamic_index_from_state(map,currentState(1), currentState(2), currentState(3), currentState(4));
    ...
    [neighbors,neighborCount]  = get_neighbors_dynamic(map, currentNode);
    ...
    for n=1:neighborCount
        ...
        [neighbor_x, neighbor_y, neighbor_vx, neighbor_vy] = dynamic_state_from_index(map,neighbors(n));
       ...
       in_closedList = 0;
        for i =1:size(closedList,1)
            if(neighbor_x == closedList(i,1) && neighbor_y==closedList(i,2) && neighbor_vx == closedList(i,3) && neighbor_vy==closedList(i,4)) 
                in_closedList = 1;
        ...
        if(~isempty(openList))% if neighbor is in openlist
            for j = 1:size(openList,1)
                if(neighbor_x == openList(j,1) && neighbor_y == openList(j,2) && neighbor_vx == openList(j,3) && neighbor_vy == openList(j,4))
                     in_openList = j;
                     break;
                end
             end
         ...
         updatedG_cost = G_cost(currentState(1),currentState(2)) + 1;  % since the cost to go from one node to another is 1 (time step)
        ...
        if(in_openList==0)
            ...
            newState = [neighbor_x neighbor_y neighbor_vx neighbor_vy  G_cost(neighbor_x, neighbor_y) updatedF_cost size(closedList,1)];
         ...
         openList(in_openList,5:7) = [updatedG_cost updatedF_cost(neighbor_x,neighbor_y) size(closedList,1)];
